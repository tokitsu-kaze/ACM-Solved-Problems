...
fun go()
{
    ...
    while(pos+d<=n)
    {
        if(res[pos+d]!=0) break
        while(now<=n&&res[now]==0) now++
        if(now==n+1)
        {
            ok=0
            break
        ...
        pos+=d
        while(pos<n&&res[pos+1]!=0) pos++
    }
    if(ok==1)
    {
        println("YES")
        var tmp_res=Array<Int>(n){0}
        for(i in 0..n-1) tmp_res[i]=res[i+1]
        println(tmp_res.joinToString(" "))
    }
    else println("NO")
}
