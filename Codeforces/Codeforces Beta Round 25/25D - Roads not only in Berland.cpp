...
int main()
{
	...
	{
		scanf("%d%d",&a,&b);
		if(!dsu.merge(a,b)) tmp.push_back({a,b,0,0});
	}
	set<int> s;
	...
	for(auto &it:tmp)
	{
		...
		{
			res.push_back({it.a,it.b,it.a,*s.begin()});
			s.erase(s.begin());
		}
		else if(dsu.merge(it.b,*s.begin()))
		{
			res.push_back({it.a,it.b,it.b,*s.begin()});
			s.erase(s.begin());
		}
		else if(dsu.merge(it.a,*s.rbegin()))
		{
			res.push_back({it.a,it.b,it.a,*s.rbegin()});
			s.erase(*s.rbegin());
		}
		else if(dsu.merge(it.b,*s.rbegin()))
		{
			res.push_back({it.a,it.b,it.b,*s.rbegin()});
			s.erase(*s.rbegin());
		}
	}
	printf("%d\n",res.size());
	for(auto &it:res) printf("%d %d %d %d\n",it.a,it.b,it.c,it.d);
	return 0;
}
