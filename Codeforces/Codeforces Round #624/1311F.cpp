...
{
	#define type ll
	int root,tot,ls[MAX*30],rs[MAX*30],ql,qr,n,cnt[MAX*30];
	type v[MAX*30],qv,c;
	int newnode()
	{
		ls[tot]=rs[tot]=0;
...
void go()
{
	...
	{
		VPII res(n);
		for(auto &it:res) read(it.se);
		for(auto &it:res) read(it.fi);
		tr.init(100000000);
		sort(all(res));
		ans=0;
		for(auto it:res)
		{
			PLL tmp=tr.ask(1,it.se);
			ans+=it.se*tmp.se-tmp.fi;
			tr.upd(it.se,it.se,it.se,1);
		}
		printf("%lld\n",ans);
	}
}
