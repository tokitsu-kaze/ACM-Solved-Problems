...
/*********************************  head  *********************************/
int dp[105][105][105][2],a[111];
void go()
{
	int n,i,j,k,cnt[2],flag[111],ans;
	while(read(n))
	{
		read(a,1,n);
		mem(flag,0);
		...
		for(i=1;i<=n;i++)
		{
			for(j=cnt[0];~j;j--)
			{
				for(k=cnt[1];~k;k--)
				{
					...
					{
						if(j) dp[i][j-1][k][0]=min({dp[i][j-1][k][0],dp[i-1][j][k][0],dp[i-1][j][k][1]+1});
						if(k) dp[i][j][k-1][1]=min({dp[i][j][k-1][1],dp[i-1][j][k][1],dp[i-1][j][k][0]+1});
					}
