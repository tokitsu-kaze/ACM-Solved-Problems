...
void go()
{
	...
	while(read(n,m))
	{
		...
		t=301;
		dc.init(t);
		for(i=1;i<sz(x);i++) dc.add_edge(s,i,x[i]-x[i-1]);
		for(i=1;i<sz(y);i++) dc.add_edge(i+100,t,y[i]-y[i-1]);
		for(i=1;i<sz(x);i++)
		{
			for(j=1;j<sz(y);j++)
			{
				for(k=0;k<m;k++)
				{
					if(x[i-1]>=xx[k].fi&&x[i]<=xx[k].se&&y[j-1]>=yy[k].fi&&y[j]<=yy[k].se)
					{
						dc.add_edge(i,j+100,LLINF);
						break;
					}
				}
			}
