...
void go()
{
	...
	while(t--)
	{
		read(n);
		read(s+1);
		VL res=get_hash_prime(2);
		ha[0].set(19260817,res[0]);
		ha[0].work(s,n);
		ha[1].set(23333333,res[1]);
		ha[1].work(s,n);
		map<PLL,int> mp;
		mp[MP(ha[0].get(3,n),ha[1].get(3,n))]=1;
		mp[MP(ha[0].get(1,n-2),ha[1].get(1,n-2))]=1;
		for(i=4;i<=n;i++)
		{
			PLL tmp;
			tmp.fi=(ha[0].get(1,i-3)*ha[0].tmp[n-i+1]%ha[0].p+ha[0].get(i,n))%ha[0].p;
			tmp.se=(ha[1].get(1,i-3)*ha[1].tmp[n-i+1]%ha[1].p+ha[1].get(i,n))%ha[1].p;
			mp[tmp]=1;
		}
		printf("%d\n",sz(mp));
	}
