...
fun go()
{
    ...
    {
        var s=readstring()
        var mp:MutableMap<Pair<Pair<Int,Int>,Pair<Int,Int>>,Int> = mutableMapOf()
        var ans=0
        var x=0
        var y=0
        for(it in s)
        {
            if(it=='N')
            {
                var tmp=mp[Pair(Pair(x,y),Pair(x-1,y))]?:0
                tmp+=mp[Pair(Pair(x-1,y),Pair(x,y))]?:0
                if(tmp>0) ans++
                else
                {
                    mp[Pair(Pair(x,y),Pair(x-1,y))]=1
                    mp[Pair(Pair(x-1,y),Pair(x,y))]=1
                    ans+=5
                }
                x--
            }
            ...
            {
                var tmp=mp[Pair(Pair(x,y),Pair(x,y-1))]?:0
                tmp+=mp[Pair(Pair(x,y-1),Pair(x,y))]?:0
                if(tmp>0) ans++
                else
                {
                    mp[Pair(Pair(x,y),Pair(x,y-1))]=1
                    mp[Pair(Pair(x,y-1),Pair(x,y))]=1
                    ans+=5
                }
                y--
            }
