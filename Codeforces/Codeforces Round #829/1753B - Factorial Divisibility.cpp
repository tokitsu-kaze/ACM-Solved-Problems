...
ll inv(ll x,ll p){return pow2(x,p-2,p);}
ll fac[2][MAX];
void go()
{
	int n,x,i,j,xx;
	...
	for(i=1;i<=MAX-10;i++)
	{
		fac[0][i]=fac[0][i-1]*i%p[0];
		fac[1][i]=fac[1][i-1]*i%p[1];
	}
	while(read(n,x))
	{
		mem(res,0);
		...
		{
			read(xx);
			res[0]=(res[0]+fac[0][xx])%p[0];
			res[1]=(res[1]+fac[1][xx])%p[1];
		}
		res[0]=res[0]*inv(fac[0][x],p[0])%p[0];
		res[1]=res[1]*inv(fac[1][x],p[1])%p[1];
	//	debug(res0,res1)
