...
ll gao(ll x,int l,int r)
{
	...
	function<ll(int,int,int,int)> dfs=[&](int pos,int lead,int sta,int limt)->ll
	{
		...
		for(int i=(limt?p[pos]:base-1);~i;i--)
		{
			...
			res+=dfs(pos-1,lead&&i==0&&pos,sta,limt&&i==p[pos]); 
		}
...
ll cal(ll l,ll r,int x,int y)
{
	mem(dp,-1);
	return gao(r,x,y)-gao(l-1,x,y);
}
void go()
{
	int t,i,j;
	...
	while(t--)
	{
		...
		for(i=0;i<=9;i++)
		{
			...
			{
				if(ans!=-1) break;
				if(cal(l,r,j,i+j)==0) continue;
	//			debug(j,i+j,cal(l,r,j,i+j))
				while(l<r)
				{
					mid=(l+r)>>1;
					if(cal(mid+1,r,j,i+j)) l=mid+1;
					else r=mid;
				}
				ans=l;
			}
