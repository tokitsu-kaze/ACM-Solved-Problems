...
void go()
{
	...
	while(read(n))
	{
		...
		while(q--)
		{
			read(v,d,x);
			res[dep[v]].pb({v,x,1});
			res[min(mx,dep[v]+d)+1].pb({v,x,-1});
		}
		for(i=1;i<=n;i++)
		{
			if(dep[id[i]]!=dep[id[i-1]])
			{
				for(auto it:res[dep[id[i]]])
				{
					tr.insert(l[it.id],it.v*it.f);
					tr.insert(r[it.id]+1,it.v*it.f*(-1));
				}
			}
			ans[id[i]]=tr.get(l[id[i]]);
		}
