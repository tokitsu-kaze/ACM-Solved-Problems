...
void go()
{
	...
	while(t--)
	{
		...
		for(i=1;i<=n;i++)
		{
			for(auto to:mp[i])
			{
				if(dis[i]+1==dis[to]) mcmf.add_edge(i*2,to*2-1,INF,0);
			}
			mcmf.add_edge(i*2-1,i*2,1,-tag[i]);
			mcmf.add_edge(i*2-1,i*2,INF,0);
			if(cnt[i]) mcmf.add_edge(i*2,T,cnt[i],0);
		}
		printf("%d\n",k+mcmf.mincost_maxflow(S,T).fi);
	}
}
