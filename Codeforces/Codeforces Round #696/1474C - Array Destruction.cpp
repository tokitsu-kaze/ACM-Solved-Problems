...
VPII ck(int p)
{
	...
	for(i=n-1;i;i--)
	{
		...
		{
			matched.erase(matched.find(a[i]));
			continue;
		}
		if(cnt[a[i]]==0) return VPII{};
		cnt[a[i]]--;
		if(cnt[now-a[i]]==0) return VPII{};
		cnt[now-a[i]]--;
		res.pb(MP(a[i],now-a[i]));
		matched.insert(now-a[i]);
		now=max(a[i],now-a[i]);
	}
	return res;
}
