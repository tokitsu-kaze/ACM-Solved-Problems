...
fun go()
{
    ...
    var m=tmp_line[1]
    var tot=m
    var all_mp:MutableMap<pair,Int> = mutableMapOf()
    var a_mp:MutableMap<Int,Int> = mutableMapOf()
    var b_mp:MutableMap<Int,Int> = mutableMapOf()
    ...
    while(m-->0)
    {
        ...
        a_mp[a]=(a_mp[a]?:0)+1
        b_mp[b]=(b_mp[b]?:0)+1
        all_mp[pair(a,b)]=(all_mp[pair(a,b)]?:0)+1
    }
    var ans=1
    for(i in 1..n)
    {
        for(j in 1..n)
        {
            ...
            if(nowa!=i&&nowb!=j)
            {
                ans=max(ans,(a_mp[i]?:0)+(b_mp[j]?:0)-(all_mp[pair(i,j)]?:0)+1)
            }
            else if(nowa!=i&&nowb==j)
            {
                ans=max(ans,(all_mp[pair(i,j)]?:0)+1)
            }
            else if(nowa==i&&nowb!=j)
            {
                ans=max(ans,(all_mp[pair(i,j)]?:0)+1)
