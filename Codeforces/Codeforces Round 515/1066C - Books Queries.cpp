...
struct Treap
{
	...
	void insert(int &id,type v,int _id)
	{
		if(!id) t[id=++tot]=node(v,_id);
		else
		{
			if(t[id].sz++,t[id].v==v)t[id].w++;
			else if(insert(t[id].ch[tmp=v>t[id].v],v,_id),t[t[id].ch[tmp]].fix>t[id].fix) rotate(id,tmp^1);
	    }
	}
	...
	void insert(type v,int id){insert(root,v,id);}
...
void go()
{
	...
	while(read(q))
	{
		...
		{
			read(op,x);
			if(op[0]=='L') t.insert(--l,x),cnt++,v[x]=l;
			else if(op[0]=='R') t.insert(++r,x),cnt++,v[x]=r;
			else printf("%d\n",min(t.lower_bound_count(v[x]),cnt-t.upper_bound_count(v[x])));
		}
	}
}