...
void go()
{
	...
	while(t--)
	{
		...
		for(i=1;i<=n;i++)
		{
			for(auto it:mp[i])
			{
				tr.upd(d.get_pos(it.l),d.get_pos(it.r),1);
			}
		}
		for(i=1;i<=n;i++)
		{
			...
			for(auto it:mp[i])
			{
				nl=d.get_pos(it.l);
				nr=d.get_pos(it.r);
				if(tr.ask(nl,nl)>0 || tr.ask(nr,nr)>0)
				{
					ans[it.id]=0;
					continue;
				}
				auto pre=s.lower_bound(it.l);
				auto nex=s.upper_bound(it.r);
				if(*pre<*nex)
				{
					ans[it.id]=0;
					continue;
				...
				pre--;
				ans[it.id]=min(*nex-it.r,it.l-*pre);
			}
			for(auto it:mp[i])
			{
				s.insert(it.l);
