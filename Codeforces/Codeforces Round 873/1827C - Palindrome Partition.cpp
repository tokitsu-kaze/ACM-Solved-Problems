...
struct Palindrome_Tree
{
	...
	int len[N],mnlen[N],nex[N][K],fail[N],last,pos[N],s[N],tot,n;
	int cnt[N],deep[N];
	int newnode(int l)
	{
	...
	void add(int t,int p)//attention the type of t is int
	{
		...
		cnt[last]++;
		pos[p]=last;
		if(len[last]%2==0) mnlen[last]=len[last];
		mnlen[last]=min(mnlen[last],mnlen[fail[last]]);
	}
	int dp[MAX];
	void work(int n)
	{
		...
		for(i=1;i<=n;i++)
		{
			dp[i]=0;
	//		debug(i,mnlen[pos[i]])
			if(mnlen[pos[i]]<=i) dp[i]=dp[i-mnlen[pos[i]]]+1;
			ans+=dp[i];
		}
		printf("%lld\n",ans);
	}
