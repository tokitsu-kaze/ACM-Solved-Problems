...
void go()
{
	...
	while(read(n))
	{
		...
		while(n--)
		{
			read(s);
			len=strlen(s);
			exkmp.work(ans+max(0,now-len),s,ex);
			res=0;
			for(j=0;j<min(len,now);j++)
			{
				if(ex[j]==min(len,now)-j)
				{
					res=ex[j];
					break;
				}
			}
	//		debug(ans+max(0,now-len),res)
			for(j=res;j<len;j++) ans[now++]=s[j];
		}
		ans[now]='\0';
		puts(ans);
	}
