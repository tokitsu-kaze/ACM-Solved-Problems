...
int ls[MAX<<5],rs[MAX<<5],size[MAX<<5],tot;
ll v[MAX<<5];
void init()
{
	ls[0]=rs[0]=0;
...
void Insert(int l,int r,int &id,int pos,int val)
{
	...
	size[id]=size[ls[id]]+size[rs[id]];
}
ll query(int l,int r,int id,int x,int y,int a)
{
	if(!id) return 0;
	if(l>=x&&r<=y)
	{
		return v[id]-(ll)a*size[id];
	}
	int mid=(l+r)>>1;
	if(y<=mid) return query(l,mid,ls[id],x,y,a);
	else if(x>=mid+1) return query(mid+1,r,rs[id],x,y,a);
	else return query(l,mid,ls[id],x,y,a)+query(mid+1,r,rs[id],x,y,a);
}
int a[MAX];
int main()
{
	...
	while(~scanf("%d",&n))
	{
		...
		for(i=n;i>=1;i--)
		{
			if(a[i]+2<=(int)1e9) ans+=query(1,(int)1e9,root,a[i]+2,(int)1e9,a[i]);
			if(a[i]-2>=1) ans+=query(1,(int)1e9,root,1,a[i]-2,a[i]);
			Insert(1,(int)1e9,root,a[i],a[i]);
		}
		cout<<ans<<"\n";
	}
	return 0;
