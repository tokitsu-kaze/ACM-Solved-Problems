...
/*------------------------------------------------------------*/
var mp=Array(200010){mutableListOf<pair>()}
var ans=Array<Int>(200010){0}
var flag=Array<Int>(200010){0}
var ok=0
fun dfs(x:Int,fa:Int)
{
    ...
    for(to in mp[x])
    {
        if(minOf(ans[x],ans[to.fi])!=to.se) ok=0
        if(flag[to.fi]==1) continue
        dfs(to.fi,x)
    }
}
fun go()
