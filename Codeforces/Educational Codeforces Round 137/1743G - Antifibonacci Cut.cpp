...
void go()
{
	int i,q,n,sum_len,len;
	ll ans,bit;
	init_fib(3e6);
	while(read(q))
	{
		vector<pair<int,ll> > res,tmp;
		res.pb(MP(0,1ll));
		sum_len=0;
		ans=1;
		bit=1;
		while(q--)
		{
			...
			for(i=1;i<=n;i++)
			{
				...
				{
					len=sum_len-it.fi;
					if(len>1&&get_kth_FibString(len)!=s[i]-'0') continue;
					if(is_fib(len)) ans=(ans-it.se+mod)%mod;
					if(len>1 || (len==1&&s[i]=='1')) tmp.pb(it);
				}
				bit=(bit+ans)%mod;
				res=tmp;
				res.pb(MP(sum_len,ans));
		//		debug(sum_len)
