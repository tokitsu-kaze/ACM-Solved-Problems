...
int main()
{
	...
	while(m--)
	{
		...
		{
			scanf("%d%d%d",&v,&u,&w);
			trg.add_edge(v,v,u,u,w);
		}
		else
		{
			scanf("%d%d%d%d",&v,&l,&r,&w);
			if(op==2) trg.add_edge(v,v,l,r,w);
			else trg.add_edge(l,r,v,v,w);
		}
	}
	trg.work(s);
	for(i=1;i<=n;i++)
	{
		if(trg.dis[i]==dij.inf) trg.dis[i]=-1;
		printf("%lld%c",trg.dis[i]," \n"[i==n]);
	}
	return 0;
}
