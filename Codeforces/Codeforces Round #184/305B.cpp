...
bigint a,b,p[111];
int n;
pair<bigint,bigint> dfs(int pos)
{
	if(pos==n-1) return MP(1,p[pos]);
	pair<bigint,bigint> tmp=dfs(pos+1);
	bigint fm=tmp.se;
	bigint fz=tmp.fi+fm*p[pos];
	bigint g=gcd(fz,fm);
	fm/=g;
	fz/=g;
//	cout<<fz<<" "<<fm<<endl;
...
int main()
{
	...
	while(cin>>a>>b)
	{
		...
		pair<bigint,bigint> res=dfs(0);
	//	cout<<res.fi<<" "<<res.se<<endl;
		if(res.fi==b&&res.se==a) puts("YES");
		else puts("NO");
	}
	return 0;
}