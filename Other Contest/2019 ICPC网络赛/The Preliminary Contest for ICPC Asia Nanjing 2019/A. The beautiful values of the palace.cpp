...
struct node
{
	int x,y,f,id;
	node(){}
	node(int _x,int _y,int _f,int _id):x(_x),y(_y),f(_f),id(_id){}
...
void go()
{
	...
	while(t--)
	{
		...
		{
			read(a,b,c,d);
			res.pb(node(a-1,b-1,1,i));
			res.pb(node(c,d,1,i));
			res.pb(node(a-1,d,-1,i));
			res.pb(node(c,b-1,-1,i));
		}
		sort(all(res));
		tr.init(n);
		...
		for(auto it:res)
		{
			if(it.f==-2) tr.insert(it.y,gao(cal(it.x,it.y,n)));
			else ans[it.id]+=it.f*tr.ask(1,it.y);
		}
