...
struct Palindrome_Tree
{
	...
	ll add(int t,int f)//attention the type of t is int
	{
		int id,now;
		s[++n]=t;
		now=get_fail(last);
		...
		last=next[now][t];
	//	if(f==2) debug(cnt[last])
		if(f==2) return cnt[last];
		if(f==0) cnt[last]++;
	}
	void count()
	{
...
void go()
{
	...
	while(t--)
	{
		...
		n=strlen(s+1);
		pam.init();
		for(i=1;i<=n;i++) pam.add(s[i]-'a',0);
		pam.count();
		read(s+1);
		n=strlen(s+1);
		pam.n=pam.last=0;
		for(i=1;i<=n;i++) pam.add(s[i]-'a',1);
		pam.build_tree();
		ans=0;
		pam.n=pam.last=0;
		for(i=1;i<=n;i++) ans+=pam.add(s[i]-'a',2);
		printf("Case #%d: %lld\n",cas++,ans);
	}
}
