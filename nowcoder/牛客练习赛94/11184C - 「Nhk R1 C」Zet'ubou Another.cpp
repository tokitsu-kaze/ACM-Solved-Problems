...
bool mp[7505][7505],flag[7505][7505];
int n,m;
void dfs(int x,int y)
{
    if(x>n||y>m) return;
    if(flag[x][y]||mp[x][y]) return;
    flag[x][y]=1;
    dfs(x+1,y);
    dfs(x,y+1);
}
void go()
{
    ...
    while(read(n,m,k))
    {
        ...
        dy.work();
        mem(mp,0);
        for(auto it:res) mp[dx.get_pos(it.fi)][dy.get_pos(it.se)]=1;
        mem(flag,0);
        n=dx.get_pos(n);
        m=dy.get_pos(m);
        dfs(dx.get_pos(0),dy.get_pos(0));
        if(flag[n][m]) puts("Yes");
        else puts("No");
    }
}
