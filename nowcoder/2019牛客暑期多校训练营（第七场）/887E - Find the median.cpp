...
struct Segment_Tree
{
	...
	int get(int x){return d.get_val(x);}
	...
	int query(int l,int r,int id)
	{
		if(l+1==r)
		{
			ll tmp=v[id]/(get(r)-get(l));
	//		debug(now,get(l),tmp)
			return get(l)+(now+tmp-1)/tmp-1;
		}
		pushdown(l,r,id);
		int mid=(l+r)>>1;
		if(v[ls]>=now) return query(l,mid,ls);
		else
		{
			now-=v[ls];
			return query(mid,r,rs);
		}
	}
...
void go()
{
	...
	while(read(n))
	{
		read(x[1],x[2],a[1],b[1],c[1],m[1]);
		read(y[1],y[2],a[2],b[2],c[2],m[2]);
		for(i=3;i<=n;i++)
		{
	...
	//		debug(l[i],r[i])
		...
		for(i=1;i<=n;i++)
		{
			tr.upd(d.get_pos(l[i]),d.get_pos(r[i])+1,1);
			printf("%d\n",tr.ask());
		}
	}
}
