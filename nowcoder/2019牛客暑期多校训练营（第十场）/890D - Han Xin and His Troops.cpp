...
bigint exgcd(bigint a,bigint b,bigint &x,bigint &y)
{
    ...
    bigint gcd=exgcd(b,a%b,x,y);
    bigint tp=x;
    x=y; y=tp-a/b*y;
    return gcd;
}
int n;
bigint ai[111],bi[111];
bigint excrt()
{
    bigint x,y,k;
    bigint M=bi[1],ans=ai[1];
    for(int i=2;i<=n;i++)
    {
        bigint a=M,b=bi[i],c=(ai[i]-ans%b+b)%b;
        bigint gcd=exgcd(a,b,x,y),bg=b/gcd;
        if(c%gcd!=0) return -1;
        x=(x*(c/gcd))%bg;
        ans+=x*M;
        M*=bg;
...
void go()
{
    ...
    while(cin>>n>>m)
    {
        for(i=1;i<=n;i++) cin>>bi[i]>>ai[i];
        ans=excrt();
        if(ans<0) puts("he was definitely lying");
