...
ll gao(int l,int r,ll x)
{
	...
	function<ll(int,int,int,int)> dfs=[&](int pos,int lead,int sta,int limt)->ll
	{
		...
		{
			int i=s[pos-l]-'0';
			int tmp=(limt?p[pos]:base-1);
			if(sta) nowlead=(lead&&i==0&&pos);
			else nowlead=lead;
			if(i<=tmp) res+=dfs(pos-1,nowlead,0,limt&&i==p[pos]); 
		}
		else
		{
			for(int i=(limt?p[pos]:base-1);~i;i--)
			{
				...
				res+=dfs(pos-1,nowlead,sta,limt&&i==p[pos]); 
			}
		}
...
ll cal(ll x)
{
	...
	ll res=0;
	len=sz(to_string(x));
	for(i=0;i+sz(s)-1<len;i++)
	{
		mem(dp,-1);
		res+=gao(i,i+sz(s)-1,x);
	}
	return res;
}
void go()
