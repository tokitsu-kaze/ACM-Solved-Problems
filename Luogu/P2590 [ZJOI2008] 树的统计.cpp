...
struct Heavy_Light_Decomposition
{
	...
	type ask_node_sum(int x,int y)
	{
		type res=0;
		while(top[x]!=top[y])
		{
			if(dep[top[x]]<dep[top[y]]) swap(x,y);
			res+=tr.ask_sum(dfn[top[x]],dfn[x]);
			x=fa[top[x]];
	    }
	    if(dep[x]>dep[y]) swap(x,y);
	    res+=tr.ask_sum(dfn[x],dfn[y]);
	    return res;
	}
	type ask_node_max(int x,int y)
	{
		type res=-INF;
		while(top[x]!=top[y])
		{
			if(dep[top[x]]<dep[top[y]]) swap(x,y);
			res=max(res,tr.ask_max(dfn[top[x]],dfn[x]));
			x=fa[top[x]];
	    }
	    if(dep[x]>dep[y]) swap(x,y);
	    res=max(res,tr.ask_max(dfn[x],dfn[y]));
	    return res;
	}
	#undef type
}hld;
...
int main()
{
	...
	{
		scanf("%s%d%d",op,&x,&y);
		if(op[0]=='C') hld.upd_node(x,x,y);
		else if(op[1]=='M') printf("%d\n",hld.ask_node_max(x,y));
		else printf("%d\n",hld.ask_node_sum(x,y));
	}
}
