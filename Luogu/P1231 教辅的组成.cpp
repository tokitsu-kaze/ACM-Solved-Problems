...
int main()
{
	int n,m,k,e,i,a,b,s,t;
	scanf("%d%d%d",&n,&m,&k);
	s=n+m+k+n+1;
	t=s+1;
	dc.init(t);
	for(i=1;i<=m;i++) dc.add_edge(s,n+i,1);
	for(i=1;i<=n;i++) dc.add_edge(i,n+m+k+i,1);
	for(i=1;i<=k;i++) dc.add_edge(n+m+i,t,1);
	scanf("%d",&e);
	while(e--)
	{
		scanf("%d%d",&a,&b);
