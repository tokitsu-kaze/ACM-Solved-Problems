...
int main()
{
	...
	t=s+1;
	dc.init(t);
	for(i=1;i<=p;i++) dc.add_edge(s,i,1);
	for(i=1;i<=q;i++) dc.add_edge(p+n+n+i,t,1);
	for(i=1;i<=n;i++) dc.add_edge(p+i,p+i+n,1);
	for(i=1;i<=n;i++)
	{
		for(j=1;j<=p;j++)
		{
			scanf("%d",&x);
			if(x) dc.add_edge(j,p+i,1);
		}
	}
	for(i=1;i<=n;i++)
	{
		...
		{
			scanf("%d",&x);
			if(x) dc.add_edge(p+n+i,p+n+n+j,1);
		}
	}
	printf("%d\n",dc.max_flow(s,t));
	return 0;
