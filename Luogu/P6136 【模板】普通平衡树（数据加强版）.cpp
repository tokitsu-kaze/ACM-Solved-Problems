...
int main()
{
	...
	while(m--)
	{
		...
		else if(op==3) lst=tr.rank(x);
		else if(op==4) lst=tr.kth(x);
		else if(op==5) lst=tr.find_pre(x);
		else if(op==6) lst=tr.find_nex(x);
		if(op>=3 && op<=6) ans^=lst;
	}
	printf("%d\n",ans);
	return 0;
}
