...
int l[MAX],r[MAX],v[MAX],a[MAX];
int main()
{
	int n,m,i,s,t,len;
	scanf("%d%d",&n,&m);
	for(i=1;i<=n;i++) scanf("%d",&a[i]);
	for(i=1;i<=m;i++) scanf("%d%d%d",&l[i],&r[i],&v[i]);
	s=n+2;
	t=s+1;
	mcmf.init(t);
	mcmf.add_edge(s,1,LLINF,0);
	mcmf.add_edge(n+1,t,LLINF,0);
	for(i=1;i<=n;i++) mcmf.add_edge(i,i+1,LLINF-a[i],0);
	for(i=1;i<=m;i++) mcmf.add_edge(l[i],r[i]+1,LLINF,v[i]);
