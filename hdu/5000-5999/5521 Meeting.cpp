...
void go()
{
	...
	for(cas=1;cas<=T;cas++)
	{
		...
		for(i=1;i<=m;i++)
		{
			...
			{
				read(x);
				dij.add_edge(x,n+i,t);
				dij.add_edge(n+i,x,0);
			}
		}
		dij.work(1);
		for(i=1;i<=n;i++) dis1[i]=dij.dis[i];
		dij.work(n);
		for(i=1;i<=n;i++) disn[i]=dij.dis[i];
		ans=LLINF;
		for(i=1;i<=n;i++) ans=min(ans,max(dis1[i],disn[i]));
		if(ans==LLINF)
		{
			puts("Evil John");
			continue;
		...
		for(i=1;i<=n;i++)
		{
			if(max(dis1[i],disn[i])==ans) res.pb(i);
		}
		printf("%lld\n",ans);
		println(res);
	}
