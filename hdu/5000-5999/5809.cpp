...
namespace kd_tree
{
	...
	struct KDT
	{
		...
		{
			if(!id) return;
			ll res=dist(t[id].x[0]-q.x[0],t[id].x[1]-q.x[1]);
			if(res!=0)
			{
				if(res<now)
				{
				...
				{
					if(t[id].x[0]<ans.x[0]) ans=t[id];
					else if(t[id].x[0]==ans.x[0])
					{
						if(t[id].x[1]<ans.x[1]) ans=t[id];
					}
				}
			}
			ll dl,dr,l,r;
		...
		inline kdtnode query(kdtnode _q){now=LLINF;q=_q;query(root);return ans;}
	}kd;
	#undef type
	#undef ls
	#undef rs
...
void go()
{
	...
	while(t--)
	{
		...
		for(i=1;i<=n;i++)
		{
			read(kd.t[i].x[0],kd.t[i].x[1]);
			kd.t[i].v=i;
		}
		kd.init(n);
		for(i=1;i<=n;i++) dsu.merge(i,(kd.query(kd.t[i])).v);
		printf("Case #%d:\n",cas++);
		while(q--)
		{
			read(a,b);
			dsu.find(a)==dsu.find(b)?puts("YES"):puts("NO");
		}
	}
}
