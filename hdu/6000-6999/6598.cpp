...
void go()
{
	...
	while(read(n,m))
	{
		...
		while(m--)
		{
			read(x,y,a,b,c);
			ans+=a+c;
			dc.add_edge(x,y+n,a/2+c/3);
			dc.add_edge(y,x+n,a/2+c/3);
			sum[x]+=a;
			sum[y]+=a;
			sum[x+n]+=c;
			sum[y+n]+=c;
		...
		for(i=1;i<=n;i++)
		{
			dc.add_edge(s,i,sum[i]);
			dc.add_edge(i+n,t,sum[i+n]);
			dc.add_edge(i,i+n,LLINF); 
		}
		printf("%lld\n",ans-dc.max_flow(s,t)/2);
	}
}
