...
struct Segment_Tree
{
	...
	#define rs (id<<1|1)
	int n,ql,qr;
	type mx[MAX<<2],lv[MAX<<2],rv[MAX<<2],v[MAX<<2],qv;
	void pushup(int id)
	{
...
void go()
{
	...
	while(t--)
	{
		...
		dx.work();
		dy.work();
		for(i=1;i<=sz(dx);i++) res[i].clear();
		for(i=1;i<=n;i++)
		{
			x[i]=dx.get_pos(x[i]);
			y[i]=dy.get_pos(y[i]);
			res[x[i]].pb(MP(y[i],w[i]));
		}
		for(i=1;i<=sz(dx);i++) sort(all(res[i]));
		ans=0;
		for(i=1;i<=sz(dx);i++)
		{
			tr.build(sz(dy));
			for(j=i;j<=sz(dx);j++)
			{
				for(auto it:res[j]) tr.upd(it.fi,it.fi,it.se);
				ans=max(ans,tr.mx[1]);
			}
		}
		printf("%lld\n",ans);
