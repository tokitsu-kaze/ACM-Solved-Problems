...
void Main()
{
	...
	while(t--)
	{
		...
		for(i=1;i<=n;i++)
		{
			...
			while(top>1&&cross(st[top-1]-st[top-2],p[i]-st[top-2])>0) top--;
			st[top++]=p[i];
		}
		debug(top);
		VI res,now,tmp;
		...
		for(i=1;i<top;i++)
		{
			if(i==top-1||cross(st[i]-st[i-1],st[i+1]-st[i])!=0) 
			{
				tmp.clear();
				tmp.pb(st[i].id);
				for(j=sz(now)-1;~j;j--)
				{
					if(now[j]<tmp[sz(tmp)-1]) tmp.pb(now[j]);
				}
				for(j=sz(tmp)-1;~j;j--) res.pb(tmp[j]);
				now.clear();
			}
			else now.pb(st[i].id);
		}
		debug(sz(res));
		for(i=0;i<sz(res);i++) print(res[i]," \n"[i==sz(res)-1]);
	}
}
