...
void Main()
{
	int t,n,q,a,b,ans[11],now,tmp,S,flag;
	char op[11];
	read(t);
	for(register int i=0;i<MAX-10;i++) cnt[i]=__builtin_popcount(i);
	while(t--)
	{
		read(n,q);
		now=1<<n;
		...
		while(q--)
		{
			...
			for(register int i=0;i<now;i++)
			{
				...
				tmp=i^S;
				dp[tmp]+=flag*dp[i];
				dp[tmp]=(dp[tmp]>=mod?dp[tmp]-mod:dp[tmp]);
				dp[tmp]=(dp[tmp]<0?dp[tmp]+mod:dp[tmp]);
			}
			for(register int i=2;i<=n;i+=2) ans[i]=0;
			for(register int i=0;i<now;i++)
			{
				if(cnt[i]&1) continue;
				ans[cnt[i]]+=dp[i];
				ans[cnt[i]]=(ans[cnt[i]]>=mod?ans[cnt[i]]-mod:ans[cnt[i]]);
			}
			for(register int i=2;i<=n;i+=2) print(ans[i]," \n"[i==n]);
		}
	}
}
