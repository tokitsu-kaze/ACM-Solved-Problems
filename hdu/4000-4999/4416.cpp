...
void go()
{
	...
	while(t--)
	{
		...
		while(n--)
		{
			...
			len=strlen(b+1);
			sam.last=1;
			for(i=1;i<=len;i++) sam.add(b[i]-'a');
		}
		ans1=0;
		for(i=1;i<=sam.tot;i++) ans1+=sam.len[i]-sam.len[sam.fa[i]];
		sam.last=1;
		len=strlen(a+1);
		for(i=1;i<=len;i++) sam.add(a[i]-'a');
		ans2=0;
		for(i=1;i<=sam.tot;i++) ans2+=sam.len[i]-sam.len[sam.fa[i]];
	//	debug(ans1,ans2)
		printf("Case %d: %lld\n",cas++,ans2-ans1);
	}
}
