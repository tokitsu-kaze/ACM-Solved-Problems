...
struct Palindrome_Tree
{
	...
	void work(char *str)
	{
		...
		for(i=1;i<=Len;i++)
		{
			t=str[i]-'A';
			s[++n]=t;
			while(x!=1&&(!next[x][t]||s[n-len[x]-1]!=s[n])) x=fail[x];
			if(next[x][t])
			{
				x=next[x][t];
				res[x]++;
		...
		for(i=tot-1;i;i--) res[fail[i]]+=res[i];
		for(i=tot-1;i;i--) ans+=1LL*res[i]*cnt[i];
	}
}pam; //pam.init(); 
char a[MAX],b[MAX];
void go()
